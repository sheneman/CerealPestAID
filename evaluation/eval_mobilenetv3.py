#!/usr/bin/env python3
"""
FACT AI - Classifier (Test Run for MobileNetV3-Large)
Evaluates a saved MobileNetV3-Large model on a held-out test set.
...
def get_mobilenet_v3_large(pretrained: bool = True):
    """Create MobileNetV3-Large with backward/forward compat for torchvision versions."""
    try:
        # Newer torchvision API (>=0.13) uses weights enum
...
def infer_num_classes_from_state(state_dict: dict) -> int | None:
    """Try to infer num_classes from final fc weight in the checkpoint."""
    # MobileNetV3-Large final layer typically at classifier.3.weight
    keys_to_try = [
        "classifier.3.weight",          # torchvision standard
        "_fc.weight",                   # sometimes used in custom heads
    ]
    for k in keys_to_try:
        if k in state_dict:
            return state_dict[k].shape[0]
    # Fallback: search for a likely final linear layer weight
    for k, v in state_dict.items():
        if k.endswith(".weight") and v.ndim == 2 and v.shape[1] in (960, 1280):
            # last layer is Linear(1280, num_classes) in standard MobileNetV3
            return v.shape[0]
    return None

def replace_classifier_head(model: nn.Module, num_classes: int):
    """Replace the final classifier layer to match num_classes."""
    # Default MobileNetV3-Large: classifier = [Linear(960,1280), Hardswish, Dropout, Linear(1280,num_classes)]
    in_features = model.classifier[3].in_features
    model.classifier[3] = nn.Linear(in_features, num_classes)

def build_model(checkpoint_path: str, num_classes_from_data: int | None = None) -> nn.Module:
    """Load MobileNetV3-Large, adjust head, and load checkpoint (supports raw state_dict or full dict)."""
    if not os.path.exists(checkpoint_path):
        raise FileNotFoundError(f"Checkpoint not found: {checkpoint_path}")

    # Load checkpoint on CPU for portability
    state = torch.load(checkpoint_path, map_location=torch.device('cpu'))
    # Training script sometimes saved raw state_dict, sometimes full dict
    if isinstance(state, dict) and "model_state_dict" in state:
        state_dict = state["model_state_dict"]
    else:
        state_dict = state

    # Infer classes from checkpoint or fallback to dataset classes
...
def make_dataloader(test_dir: str, batch_size: int, num_workers: int):
    """Create a deterministic test loader and return class names and dataset."""
...
def evaluate(model, loader, device, class_names, save_csv: str | None):
    ...
    print(f"\n=== Test Results (MobileNetV3-Large) ===")
...
def parse_args():
    p = argparse.ArgumentParser(description="Test MobileNetV3-Large classifier on a test split.")
    p.add_argument("--test-dir", default="./data/labels/test", help="Path to test split (ImageFolder).")
    p.add_argument("--checkpoint", default="./models/best_factai_mobilenetv3.pth",
    ...
    p.add_argument("--csv-out", default="./models/test_predictions_mobilenetv3.csv",
...
def main():
    ...
    # Build & load model (handles raw state_dict vs full dict)
